...
def write_report_html3(node_df, edge_df, path_list):
    if len(path_list) != 0:
        node_list = []
        for idx, (path_ids, path_labels) in enumerate(path_list, 1):
            for node_id in path_ids:
    ...
    for node_type, style in NODE_STYLES.items():
        subset = node_df[node_df['node_type'] == node_type]
        for _, r in subset.iterrows():
            # build kwargs depending on whether shape is an icon
            kwargs = {}
            shape = style.get('shape', 'icon')
            if shape == 'icon':
                kwargs['shape'] = 'icon'
    ...
    # Add Edges (unchanged)
    for _, r in edge_df.iterrows():
        net.add_edge(r['source'], r['target'], title=r.get('relationship'), color='blue')
...
def write_report_html2(node_df, edge_df, path_list):
    if len(path_list) != 0:
        node_list = []
        for idx, (path_ids, path_labels) in enumerate(path_list, 1):
            for node_id in path_ids:
    ...
    #add Nodes
    students_df = node_df[node_df['node_type'] == 'Student']
    for i,r in students_df.iterrows():
        net.add_node(r['node_id'], label=r['label'], title=r['label'], shape="icon",
                     icon={"face": "FontAwesome", "code": "\uf007", "size": 50, "color": "blue"})
    teachers_df = node_df[node_df['node_type'] == 'Teacher']
    for i,r in teachers_df.iterrows():
        net.add_node(r['node_id'], label=r['label'], title=r['label'], shape="icon",
                     icon={"face": "FontAwesome", "code": "\uf19d", "size": 50, "color": "green"})
    classes_df = node_df[node_df['node_type'] == 'Class']
    for i,r in classes_df.iterrows():
        net.add_node(r['node_id'], label=r['label'], title=r['label'], shape="icon",
                     icon={"face": "FontAwesome", "code": "\uf02d", "size": 50, "color": "red"})
    address_df = node_df[node_df['node_type'] == 'Address']
    for i,r in address_df.iterrows():
        net.add_node(r['node_id'], label=r['label'], title=r['label'], shape="icon",
                     icon={"face": "FontAwesome", "code": "\uf015", "size": 50, "color": "purple"})
    # Add Edges
    enrolled_in_df = edge_df[edge_df['relationship'] == 'ENROLLED_IN']
    for i,r in edge_df.iterrows():
        net.add_edge(r['source'], r['target'], title=r['relationship'], color='blue')


    html_path = "output/my_graph.html"
    net.save_graph(html_path)

    # 3) Inject Font CSS into the HTML (so the iframe has them)
    fa_link = '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">'
